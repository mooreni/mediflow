"""Vertex AI client factories for MediFlow.
...
eagerly and must not be shared across threads that call ``asyncio.run()`` concurrently.
Always use a thread-local factory inside ``ThreadPoolExecutor``.
"""
...
# Each thread gets its own namespace so async HTTP pools are never shared.
